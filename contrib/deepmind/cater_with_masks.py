# From https://github.com/deepmind/multi_object_datasets/blob/master/cater_with_masks.py
...
def feature_descriptions(sequence_length=SEQUENCE_LENGTH, max_num_entities=MAX_NUM_ENTITIES):
    return {
        "camera_matrix": tf.io.FixedLenFeature([sequence_length, 4, 4], tf.float32),
        "image": tf.io.FixedLenFeature([], tf.string),
        "mask": tf.io.FixedLenFeature([], tf.string),
        "object_positions": tf.io.FixedLenFeature([max_num_entities, sequence_length, 3], tf.float32),
    }


def _decode(example_proto, features, sequence_length=SEQUENCE_LENGTH, max_num_entities=MAX_NUM_ENTITIES):
    ...
    single_example["image"] = tf.reshape(single_example["image"], [sequence_length] + IMAGE_SIZE + [3])
    single_example["mask"] = tf.reshape(single_example["mask"], [sequence_length, max_num_entities] + IMAGE_SIZE + [1])
    single_example["object_positions"] = tf.transpose(single_example["object_positions"], [1, 0, 2])
